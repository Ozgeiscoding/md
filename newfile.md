...
### 3. Simülasyonu Başlatma (Bringup)

`robotaksi_bringup` paketi ile tüm sistemi tek komutla başlatabilirsiniz.
Bu komut; Gazebo, robot modeli, kontrolcüler, lokalizasyon (AMCL, EKF) ve Nav2 yığınını çalıştırır.
...
> Not: Eğer `robotaksi_bringup` zaten navigation başlatıyorsa bu komutu ayrıca çalıştırmayın, node çakışması olur.
